...
class Blob:
    ...
    def __init__(
            self, centroid, convexHull,
            convexHullArea, contour, contourArea
    ):
...
def getBlobs(BW, minSize, maxSize = None):
    ...
    for con in cs:
        ...
        large_enough = abs(cv2.contourArea(con)) > minSize
        small_enough = abs(cv2.contourArea(con)) < _maxSize

        if large_enough and small_enough:
            m = cv2.moments(con)
            try:
                # image moments
                m10 = m['m10']
